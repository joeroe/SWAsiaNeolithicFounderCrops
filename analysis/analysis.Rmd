...
Number & percentage of assemblages with founder crops reported, by period (excluding taxa with indeterminate "founder crop" status:

```{r occ-founder}
flora %>% 
  drop_na(founder_crop) %>% 
  group_by(period, phase_code) %>% 
  summarise(founder_crop = any(founder_crop), .groups = "drop_last") %>% 
...
flora_ts %>% 
  ...
  summarise(founder_crop = any(founder_crop, na.rm = TRUE), .groups = "drop") %>% 
  group_by(century) %>% 
  summarise(prop = sum(founder_crop) / n()) %>% 
  ggplot(aes(century, prop)) +
  geom_area() +
  scale_x_reverse() +
  labs(x = "cal BP", y = "Proportion of assemblages with founder crops") +
  theme(legend.position = "bottom")
