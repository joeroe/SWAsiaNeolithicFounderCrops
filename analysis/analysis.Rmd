...
```{r occ-all-plot, fig.height=12, fig.cap='Occurrence of taxa in Neolithic assemblages (founder crops highlighted).', layout='l-body-outset'}
flora %>% 
  drop_na(category, taxon) %>% 
  filter(period %in% neolithic) %>% 
  group_by(category, taxon, founder_crop, phase_code) %>% 
...
Table \@ref(tab:category-summary) summarises the ubiquity and abundance of the broad plant categories across Neolithic assemblages, \@ref(tab:category-summary-byperiod) the same data broken down by period, and figure \@ref(fig:category-ts) how this changed through time.
...
# ggsave(here("analysis/figures/category-ts.pdf"), width = 160, height = 160,
       # units = "mm")
```
