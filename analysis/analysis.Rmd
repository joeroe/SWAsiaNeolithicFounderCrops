...
# TODO: NE broken. Again.

land <- st_union(rnaturalearth::ne_countries(scale = 50, returnclass = "sf"))
...
p1_left <- p1 + facet_wrap(vars("All"))
p1_right <- p1 + 
...
flora %>% 
  ...
  mutate(
    pct_present = round(n_present / n_phases * 100, 2),
    pct_gthalf = round(n_gthalf / n_phases * 100, 2),
    pct_gtqrtr = round(n_gtqrtr / n_phases * 100, 2)
  ) %>% 
  write_csv("occ_all.csv")
  # slice_max(n_present, n = , with_ties = FALSE)
```

### Founder crops

Number & percentage of assemblages with *all* the founder crops reported, by period (excluding taxa with indeterminate "founder crop" status:

```{r occ-founder}
flora %>% 
  group_by(period, phase_code) %>% 
  summarise(founder_crop = any(!is.na(founder_crop)), .groups = "drop_last") %>% 
