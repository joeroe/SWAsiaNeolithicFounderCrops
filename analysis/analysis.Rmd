...
# TODO: final cleaning
# - drop entries where n or p = 0,
# - drop entries where p is NA
# - drop entries where (canonical) taxon is NA
...
flora %>% 
  ...
  gt("Occurrence of all taxa in Neolithic assemblages (founder crops in bold)") %>% 
...
We can show the same data graphically:

```{r occ-all-plot, fig.cap='Occurrence of taxa in Neolithic assemblages (founder crops highlighted).', layout='l-body-outset'}
flora %>% 
  drop_na(category, taxon) %>% 
  filter(period %in% neolithic) %>% 
  group_by(category, taxon, founder_crop, phase_code) %>% 
  ...
  mutate(taxon = fct_reorder(taxon, n_present, .desc = TRUE)) %>% 
  ggplot(aes(taxon, pct_present, fill = !is.na(founder_crop))) +
  facet_wrap(vars(category), nrow = 1, scales = "free_x") +
  geom_col(colour = "black") +
  scale_x_discrete(labels = function(x) stringr::word(x, 1, 2)) +
  scale_y_continuous(labels = scales::label_percent()) +
  scale_fill_manual(values = c("white", "black"), guide = FALSE) +
  labs(x = NULL, y = "Assemblages present", 
       caption = "Includes only the 10 most ubiquitous taxa in each category.") +
  theme_minimal() +
  theme(
...
TODO: specific figures/tables/analyses?
