...
This document describes the quantitative analysis supporting our paper:

> Arranz-Otaegui, Amaia and Roe, Joe. in prep. Revisiting the concept of the “Neolithic Founder Crops” in southwest Asia. Submitted to *Vegetation History and Archaeobotany*.

It is generated from the compendium of R code and data available at *insert Zenodo link* or as a git repository at <https://github.com/joeroe/SWAsiaNeolithicFounderCrops>.
...
We used this data to quantitatively assess early evidence for translocation of certain crops from Southwest Asia to Europe, reported in table X of the main text.
...
ggsave(here("analysis/figures/fig1.eps"), width = 174, height = 116,
       units = "mm")
ggsave(here("analysis/figures/fig1.png"), width = 174, height = 116,
       units = "mm", dpi = 300)
```
...
```{r all-assemblages, layout='l-body-outset'}
# flora_phases %>% 
#   select(
#     site_name,
#     phase,
...
flora %>% 
  ...
  coord_flip() +
  scale_x_discrete(labels = function(x) str_trunc(x, 100)) +
...
flora_ts %>%
  ...
  mutate(founder_crops = str_replace(founder_crops, "atleast", "More than ")) %>% 
  # Plot
  ggplot() +
...
flora_ts %>%
  ...
  scale_y_continuous(limits = c(0, 0.5), breaks = scales::breaks_width(0.1),
                     ...
                     expand = expansion(mult = c(0, 0.05))) +
  khroma::scale_fill_muted() +
  labs(x = "ka cal BP", y = "Mean proportion of assemblages", fill = "Taxon") +
  theme_half_open(font_size = 9) ->
  p2
